...
`REGION_CODE` is the same lookup keyed to the ONS region CODE (E12xxxxxx)
rather than the name - needed to join against `hpi.build.HPIResult.
region_factors` (`region_code`, the same E12xxxxxx scheme, already used
for the IFS regional-ratio validation anchors in hpi/build.py) for the
HPI-geography sensitivity axis, where `REGION`'s human-readable names
would need an extra lookup step to be usable directly.
"""
