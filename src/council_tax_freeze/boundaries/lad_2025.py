"""
The 296 English local authority districts as at 1 April 2025 - the target
vintage every source unit in reorg_events.py should ultimately resolve to.

Committed directly (small, stable, cited reference data) rather than
downloaded into gitignored data/, for the same reason reorg_events.py is
committed: the boundary harmonisation tests need to check resolutions
against ground truth without requiring a full raw-data download step first.

Source: ONS Open Geography Portal, "Local Authority Districts (April 2025)
Names and Codes in the UK (V2)", ArcGIS item 5779a9578f0e48ccacef6af41546b56b,
FeatureServer layer LAD_APR_2025_UK_NC_v2, queried directly (the Hub dataset
page itself is JS-rendered and exposes no static download link, same
situation as the Code History Database - see reorg_events.py). Filtered to
LAD25CD starting 'E' (England only, per project scope); UK total was 361,
England is these 296. Every successor code used anywhere in reorg_events.py
was cross-checked against this list and matches name-for-name.
"""

from __future__ import annotations

LAD_2025: dict[str, str] = {
...
LAD_2025_CODES: frozenset[str] = frozenset(LAD_2025)

assert len(LAD_2025) == 296, f"expected 296 English LADs as at April 2025, got {len(LAD_2025)}"
